...
function dp_fill!(q::AbstractBytes, t::AbstractBytes, F::Matrix{Int}, P::Matrix{Byte}, mm=7, go=11, ge=2)::Void
    F[1, :] = t .== q[1]
    P[1, :] = CLIP + DIAG * (t .== q[1])
...
function write_alignment(io::IO, start_pos, q, t, l, buf)
    i, j = start_pos
    for k in 1:l
        if buf[1, k]
            io << q[i]
...
"""
function dp_report(q::AbstractBytes, t::AbstractBytes, F::Matrix{Int}, P::Matrix{Byte}, th::Int=64)
    ...
    for i in 1:length(q)-th, j in 1:length(t)-th # skip those shorter than th
        if F[i, j] >= th && P[i, j] & CLIP != 0
            ...
            str = sprint(write_alignment, start_pos, q, t, l, buf)
...
# "Align two sequences"
# @main function main(a::String, b::String; mismatch::Int=7, gap_open::Int=11, gap_extend::Int=2, threshold::Int=64)
#     a, b = a.data, b.data
