...
const c_notify_fun = cfunction(notify_fun, Void, (Cint,))
...
"""
macro tcall(f, rettype, argtypes, argvals...)
...
function do_threadcall(wrapper::Function, rettype::Type, argtypes::Vector, argvals::Vector)
    ...
    for (T, x) in zip(argtypes, argvals)
        y = Base.cconvert(T, x)
        push!(roots, y)
        unsafe_store!(convert(Ptr{T}, ptr), Base.unsafe_convert(T, y))
        ptr += sizeof(T)
    end
    ...
    Base.acquire(threadcall_restrictor)
    ...
    Base.release(threadcall_restrictor)
